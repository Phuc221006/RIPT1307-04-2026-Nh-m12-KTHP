...
export const EducationController = {
  ...
  createUni: async (req: Request, res: Response) => {
    ...
    const uni = await prisma.universities.create({ data: { id: crypto.randomUUID(), code, name, description } });
  ...
  createMajor: async (req: Request, res: Response) => {
    ...
    const major = await prisma.majors.create({ data: { id: crypto.randomUUID(), university_id, code, name } });
  ...
  createCombo: async (req: Request, res: Response) => {
    ...
    const combo = await prisma.subject_combinations.create({ data: { id: crypto.randomUUID(), major_id, code, subjects } });
